...
*/
func FixedLengthCombinations(arrayLength int, groupLength int, upTo bool) <-chan []int {
	ch := make(chan []int)
	...
	go func(ch chan []int) {
		defer close(ch)
		add(ch, []int{}, items, groupLength, upTo)
	}(ch)
	return ch
}

func add(ch chan []int, current []int, items []int, groupLength int, upTo bool) {
	if len(current) == groupLength {
		ch <- current
		return
	} else if upTo && len(current) < groupLength {
		ch <- current
	}
	for i, item := range items {
		add(ch, append(current, item), items[i+1:], groupLength, upTo)
	}
}
