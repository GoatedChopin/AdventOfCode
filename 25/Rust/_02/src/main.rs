...
fn test() {
  ...
  let negatives = vec![12, 95, 98, 115, 1188511880, 222221, 446443, 38593856, 565653, 824824821];
  for negative in negatives {
    assert!(!is_equal_halves(negative));
  }
  let input = read_input("test.txt");
  assert_eq!(part_one(&input, false), 11 + 22 + 99 + 1010 + 1188511885 + 222222 + 446446 + 38593859);
